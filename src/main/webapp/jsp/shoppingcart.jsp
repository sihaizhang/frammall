...
<script type="text/javascript">
    ...
    function showMyCartList(result) {
        ...
        $.each(cartList, function (index, cartItem) {
            var choiceTd = $("<td></td>").append($("<label></label>").append("选择")).attr("cart_id", cartItem.id);
            ...
            var imageTd = $("<td></td>").append($("<a href='#'></a>").append($("<img alt='#' class='img-thumbnail' style='width:100px;height: 120px;'>").attr("src", cartItem.mainImage)));
            var productNameTd = $("<td></td>").append($("<div></div>").append($("<a href='#' class='show_product_detail'></a>").append(cartItem.name).attr("product_id", cartItem.productId)));
            ...
            var quantityTd = $("<td></td>").append($("<div class='btn-group' style='text-align:center;'></div>").append(quantityDec).append(quantityInput).append(quantityAdd));
            ...
            var priceTd = $("<td></td>").append($("<div>单价：</div>").append(priceInput));
            var totalPriceTd = $("<td></td>").append($("<div>小记：</div>").append($("<span id='totalPrice'></span>").append(cartItem.price * cartItem.quantity)));

            var deleteBtn = $("<button class='btn btn-danger'>删除</button>").attr("cart_id", cartItem.id);
            var orderBtn = $("<button class='btn btn-info'>付款</button>");
            var operaTd = $("<td></td>").append($("<div></div>").append(deleteBtn).append(orderBtn));
    ...
    function showShipping(result) {
        ...
        $.each(shippingList, function (index, shippingItem) {
            ...
            var hahaha = $("<a class='address-item active'></a>").attr("shipping_id", shippingItem.id);
